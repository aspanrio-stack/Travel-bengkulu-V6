 ...
 *
 * FIX: sanitize() membersihkan karakter non-ASCII (seperti arrow →)
 *      sebelum di-render ke PDF agar tidak error WinAnsi encode.
 *      PDF attachment dikirim sebagai base64 string ke Resend.
 */
...
function sanitize(text: string): string {
  return (text || '')
    ...
    .replace(/"/g, '"')
    .replace(/…/g, '...')
    // Hapus semua karakter di luar range WinAnsi (0x00-0xFF)
    .replace(/[^\x00-\xFF]/g, '');
}
...
async function generateTicketPDF(order: {
  ...
  // Tampilkan rute pakai '->' (ASCII aman, tidak error WinAnsi)
  ...
  page.drawText(sanitize(formatRp(order.total)), { x: margin + 16, y: y - 34, size: 22, font: fontBold, color: white });
  y -= 68;
