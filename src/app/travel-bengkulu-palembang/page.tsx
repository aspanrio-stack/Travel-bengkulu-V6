import type { Metadata } from 'next';
import Link from 'next/link';
import ArticleLayout from '@/components/ArticleLayout';

export const metadata: Metadata = {
  title: 'Travel Bengkulu Palembang – Antar Jemput Rp 250.000 ongkos murah 2026',
  description:
    'Bengkulu Palembang door to door. Rp 250.000/orang. HiAce lebar, Innova. Berangkat hari ini bisa bayar pakai QRIS!',
  alternates: { canonical: 'https://bengkulutravel.com/travel-bengkulu-palembang' },
};

export default function TravelBengkuluPalembang() {
  return (
    <ArticleLayout
      title="Travel Bengkulu Palembang"
      description="Layanan antar jemput door to door Bengkulu–Palembang dengan tarif terjangkau dan armada nyaman."
      breadcrumbs={[{ label: 'Travel Bengkulu Palembang' }]}
      badge="🚗 Rute Populer"
      price="Rp 250.000"
    >
      {/* Tombol Pesan Cepat */}
      <div className="mb-6 p-4 bg-primary-50 border border-primary-200 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-3">
        <p className="text-primary-800 font-semibold text-sm">Siap memesan? Klik tombol di bawah untuk langsung ke form pemesanan.</p>
        <Link
          href="/pesan?rute=bkl-plm"
          className="shrink-0 bg-primary-600 hover:bg-primary-700 text-white font-bold px-6 py-2.5 rounded-xl transition-colors flex items-center gap-2 text-sm"
        >
          🎫 Pesan Sekarang
        </Link>
      </div>
      <p>
        <strong>Travel Bengkulu Palembang</strong> adalah layanan transportasi antar kota yang melayani rute Bengkulu menuju Palembang (dan sebaliknya) dengan sistem <em>door to door</em>. Artinya, kami akan menjemput Anda langsung dari rumah, kantor, atau hotel Anda, kemudian mengantarkan ke tujuan di Palembang tanpa perlu berpindah kendaraan.
      </p>

      <h2>Tarif Travel Bengkulu Palembang</h2>
      <table>
        <thead>
          <tr>
            <th>Rute</th>
            <th>Tarif per Orang</th>
            <th>Keterangan</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>Bengkulu → Palembang</td>
            <td><strong>Rp 250.000</strong></td>
            <td>Door to door</td>
          </tr>
          <tr>
            <td>Palembang → Bengkulu</td>
            <td><strong>Rp 250.000</strong></td>
            <td>Door to door</td>
          </tr>
        </tbody>
      </table>

      <h2>Armada yang Digunakan</h2>
      <p>
        Untuk kenyamanan perjalanan Bengkulu–Palembang, kami menyediakan armada:
      </p>
      <ul>
        <li><strong>Toyota Innova</strong> – Pilihan premium untuk keluarga. Suspensi lebih halus dan kabin lebih lapang.</li>
        <li><strong>Toyota HiAce</strong> – Ideal untuk rombongan besar 10–13 orang. Bisa disewa secara khusus.</li>
      </ul>

      <h2>Rute Perjalanan Bengkulu ke Palembang</h2>
      <p>
        Perjalanan dari Bengkulu ke Palembang ditempuh melalui jalur Trans-Sumatera dengan jarak sekitar 460 km. Estimasi waktu tempuh sekitar <strong>8–10 jam</strong> tergantung kondisi jalan dan cuaca.
      </p>
      <p>
        Rute yang biasa dilalui: <em>Bengkulu → Kepahiang → Curup → Lubuklinggau → Lahat → Prabumulih → Palembang</em>.
      </p>

      <h2>Jadwal Keberangkatan</h2>
      <p>
        Kami beroperasi setiap hari dan memiliki jadwal keberangkatan fleksibel. Tersedia keberangkatan pagi, siang, dan malam hari. Jadwal dapat disesuaikan dengan kebutuhan Anda, terutama untuk pemesanan rombongan.
      </p>

      <h2>Cara Pemesanan</h2>
      <ol>
        <li>Hubungi kami via WhatsApp di nomor <strong>0852-6864-5461</strong></li>
        <li>Beritahukan tanggal berangkat dan jumlah penumpang</li>
        <li>Konfirmasi titik jemput dan tujuan di Palembang</li>
        <li>Kami konfirmasi ketersediaan dan detail pembayaran</li>
        <li>Pada hari H, driver kami akan tiba tepat waktu di lokasi Anda</li>
      </ol>

      <h2>Fasilitas Selama Perjalanan</h2>
      <ul>
        <li>AC dingin dan berfungsi dengan baik</li>
        <li>Kursi nyaman dan rapi</li>
        <li>Pengemudi berpengalaman dan ramah</li>
        <li>Berhenti di rest area untuk istirahat dan makan</li>
        <li>Bagasi tidak dibatasi (sesuai kapasitas kendaraan)</li>
      </ul>

      <h2>Tips Perjalanan Bengkulu – Palembang</h2>
      <ul>
        <li>Pesan minimal 1 hari sebelum keberangkatan, terutama di musim liburan</li>
        <li>Siapkan bekal minuman dan camilan untuk perjalanan panjang</li>
        <li>Informasikan kepada kami jika ada penumpang yang mabuk perjalanan</li>
        <li>Bawa dokumen penting seperti KTP atau kartu keluarga</li>
      </ul>

      <h2>Mengapa Pilih Travel Bengkulu Palembang Kami?</h2>
      <p>
        Kami telah melayani ribuan penumpang di rute Bengkulu–Palembang dengan rekam jejak yang terpercaya. Harga Rp 250.000 sudah termasuk jemputan dari rumah Anda hingga diantar ke tujuan di Palembang. Tidak perlu khawatir tentang biaya tambahan atau ongkos naik ojek ke terminal.
      </p>

      <h2>FAQ Travel Bengkulu Palembang</h2>

      <h3>Bengkulu ke Palembang berapa jam perjalanannya?</h3>
      <p>
        Perjalanan Bengkulu–Palembang via travel darat membutuhkan waktu sekitar <strong>8–10 jam</strong> dengan jarak tempuh ±460 km. Waktu ini sudah termasuk 1–2 kali berhenti di rest area.
      </p>

      <h3>Bengkulu ke Palembang berapa km?</h3>
      <p>
        Jarak Bengkulu ke Palembang via jalur darat sekitar <strong>460 km</strong> melewati Kepahiang, Curup, Lubuklinggau, Lahat, dan Prabumulih.
      </p>

      <h3>Apakah travel berangkat setiap hari?</h3>
      <p>
        Ya, kami melayani rute Bengkulu–Palembang <strong>setiap hari</strong>. Tersedia jadwal pagi, siang, dan malam sesuai kebutuhan Anda.
      </p>

      <h3>Apakah ada biaya tambahan selain Rp 250.000?</h3>
      <p>
        Tidak ada biaya tambahan. Tarif Rp 250.000 sudah <strong>all-in</strong> termasuk penjemputan di alamat Anda dan pengantaran ke tujuan di Palembang.
      </p>

      <h3>Berapa lama sebelumnya harus pesan?</h3>
      <p>
        Disarankan pesan minimal <strong>1 hari sebelum keberangkatan</strong>. Untuk musim liburan, lebih baik pesan 2–3 hari sebelumnya agar kursi tersedia.
      </p>

      <h3>Bisa muat berapa koper/bagasi?</h3>
      <p>
        Untuk Avanza dan Innova muat 2–3 koper besar. Untuk HiAce muat lebih banyak. Informasikan jumlah bagasi saat pemesanan agar kami siapkan kendaraan yang sesuai.
      </p>
    </ArticleLayout>
  );
}
