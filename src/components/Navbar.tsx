...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <div className="leading-tight">
            ...
            <div className="font-display font-bold text-primary-700 text-sm -mt-0.5">Bengkulu</div>
          </div>
        </Link>
          ...
          {navLinks.map((link) =>
            link.children ? (
              <div key={link.label} className="relative group">
                ...
                {dropdown === link.label && (
                  <div
                    className="absolute top-full left-0 mt-1 bg-white rounded-xl shadow-lg border border-slate-100 py-2 min-w-[240px] z-50"
                    onMouseEnter={() => setDropdown(link.label)}
                    onMouseLeave={() => setDropdown(null)}
                  >
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            ...
            className="inline-flex items-center gap-1.5 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold px-3 py-2 md:px-4 md:py-2 rounded-xl transition-all"
          >
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="md:hidden bg-white border-t border-slate-100 px-4 py-4 space-y-2 max-h-[80vh] overflow-y-auto">
          {navLinks.map((link) =>
            link.children ? (
              <div key={link.label}>
                <button
                  onClick={() => setExpandedMobile(expandedMobile === link.label ? null : link.label)}
                  className="w-full flex items-center justify-between py-2 px-3 text-sm font-semibold text-slate-500 uppercase tracking-wider hover:bg-slate-50 rounded-lg"
                >
                  {link.label}
                  <svg className={`w-4 h-4 transition-transform ${expandedMobile === link.label ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
